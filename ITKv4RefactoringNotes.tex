...
\caption{\baselineskip 12pt \small The standard approach in the ITKv4
  ...
  between the images as in (c).  Second, we remove the shape (diffeomorphic)
  differences (d).  This results in a composite mapping, computed via the
...
The metric (or comparison function between images) is then defined completely as $M(I,J,\phi(x,p))$.  For instance, $M=\|
...
\begin{eqnarray}
 M_{p_i}=\frac{\partial M}{\partial
  ...
  J(\phi(x,p))}{\partial \phi} \frac{\partial \phi}{\partial p_i}|_x
~~.
\label{eq:grad}
\end{eqnarray}
