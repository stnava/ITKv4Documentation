...
The second stage computes a deformation, $\rightsquigarrow$, from $J \rightarrow $
...
\end{tabular}
\caption{\baselineskip 12pt \small The standard approach in the ITKv4
  registration framework is to map image $J$ to image $I$ by first
  identifying the linear transformation that removes (a sub-space of) affine variation
  between the images.  Second, we remove the shape (diffeomorphic)
  differences.  This results in a composite mapping, computed via the
...
\caption{\baselineskip 12pt \small An ITK diffeomorphic mapping of the
  type $(J \leftrightsquigarrow) \approx I$.  The 
...
\caption{We define an affine schema as $I ~\approx_\text{mi}  \rightarrow
J_i $ for affine mapping a set of $\{ J_i \}$ images
to a template $I$.  We also define a composite schema as $I
~\approx_\text{cc}  \leftrightsquigarrow \approx_\text{mi}  \rightarrow
J_i $ for affine mapping a set of $\{ J_i \}$ images
