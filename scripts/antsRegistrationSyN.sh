...
while getopts "d:f:h:m:n:o:p:r:s:t:x:" OPT
  do
  case $OPT in
      h) #help
   Help
...
RIGIDCONVERGENCE="[1000x500x250x10,1e-6,10]"
...
SYNCONVERGENCE="[200x100x50x20,0,10]"
...
for(( i=0; i<${#FIXEDIMAGES[@]}; i++ ))
  do
    if [[ ${METRICTYPE} == "MI" ]] ; then
      SYNMETRICS="$SYNMETRICS --metric MI[${FIXEDIMAGES[$i]},${MOVINGIMAGES[$i]},1,32]"
    else 
